...
def nlp_assistant():
    ...
    return jsonify(chatbot.answer(db, user_query)), 200
...
def demand_forecast(current_user):
    ...
    The scikit-learn time-series pipeline lives in forecast.py.
    """
    payload = request.json or {}
    target_item_id = payload.get("item_id")

    result = forecast.forecast_demand(db, target_item_id)
    if result is None:
